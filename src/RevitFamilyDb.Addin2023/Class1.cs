...
namespace RevitFamilyDb.Addin2023;
...
public class App : IExternalApplication
{
    public Result OnStartup(UIControlledApplication application)
    {
        ...
        var tools = panel.AddItem(new PulldownButtonData("FamilyDbTools2023", "DB")) as PulldownButton;
        if (tools != null)
        {
    ...
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
        try
        {
            ...
            foreach (var fam in new FilteredElementCollector(doc).OfClass(typeof(Family)).Cast<Family>())
            {
                ...
                {
                    FamilyName = string.IsNullOrWhiteSpace(fam.Name) ? "UnnamedFamily" : fam.Name,
                    CategoryName = fam.FamilyCategory == null ? "N/A" : fam.FamilyCategory.Name,
                    RfaPath = BuildPseudoLoadablePath(doc.PathName, fam.Name),
                    RevitVersion = revitYear,
                    ApprovalStatus = "Draft"
                });
                added++;
            }
            foreach (var typ in new FilteredElementCollector(doc).WhereElementIsElementType().Cast<ElementType>().Where(x => x.Category != null))
            {
                repo.UpsertFamily(new FamilyRecord
                {
                    FamilyName = string.IsNullOrWhiteSpace(typ.FamilyName) ? typ.Name : typ.FamilyName + " : " + typ.Name,
                    CategoryName = typ.Category?.Name ?? "System",
                    RfaPath = BuildPseudoSystemPath(doc.PathName, typ.Id.IntegerValue),
                    RevitVersion = revitYear,
                    ApprovalStatus = "Draft"
                });
                added++;
